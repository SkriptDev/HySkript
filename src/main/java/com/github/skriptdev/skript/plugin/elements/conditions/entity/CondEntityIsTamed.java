...
public class CondEntityIsTamed extends PropertyConditional<LivingEntity> {

    public static void register(SkriptRegistration reg) {
        reg.newPropertyConditional(CondEntityIsTamed.class, "livingentities",
                ConditionalType.BE, "tamed")
            .name("Entity is Tamed")
    ...
    public boolean check(@NotNull TriggerContext ctx) {
        return this.getPerformer().check(ctx, entity -> {
            if (entity instanceof NPCEntity npcEntity) {
                return EntityUtils.isTamed(npcEntity);
            }
            return false;
        }, isNegated());
    }
