...
public class ExprPlayerMovementSwimJumpForce extends PropertyExpression<Player, Number> {

    public static void register(SkriptRegistration reg) {
        reg.newPropertyExpression(ExprPlayerMovementSwimJumpForce.class, Number.class,
                "swim jump force", "players")
            .name("Player Movement - Swim Jump Force")
            .description("Get/change the swim jump force of a player.",
                "This is not persistent. Default = 10.0")
            .examples("set swim jump force of player to 5",
                "reset swim jump force of player")
            .since("1.1.0")
            .register();
    }
    ...
    public void change(@NotNull TriggerContext ctx, @NotNull ChangeMode changeMode, Object @NotNull [] changeWith) {
        ...
        for (Player player : getOwner().getArray(ctx)) {
            ...
            if (changeMode == ChangeMode.RESET) {
                component.getSettings().swimJumpForce = component.getDefaultSettings().swimJumpForce;
            } else if (number != null) {

                float previousValue = component.getSettings().swimJumpForce;
                float changeValue = number.floatValue();

                component.getSettings().swimJumpForce = switch (changeMode) {
