...
public class CondPlayerMovementRunning extends PropertyConditional<Player> {

    public static void register(SkriptRegistration reg) {
        reg.newPropertyConditional(CondPlayerMovementRunning.class,
                "players", ConditionalType.BE, "running")
            .name("Player Movement - Running")
    ...
    public boolean check(@NotNull TriggerContext ctx) {
        return this.getPerformer().check(ctx, player -> {
            ...
            return component.getMovementStates().running;
        }, isNegated());
    }
